...
- 概览优先展示系统健康、异常和待处理事项；信息按「今日运行 / 历史累计 / 最近 14 天」分层，历史数据直接从业务表按站点时区聚合并为无记录日期补零，避免累计运营记录只存在于数据库却无法在概览查看。色彩只用于状态、风险和主要动作，不新增装饰性卡片墙。
- 后台使用独立的 compact density 作用域，不降低生成工作台与营销页面的触控尺寸。
- 模块、筛选和设置分类写入 `#/admin?tab=...`，支持刷新、前进后退与深链接。
