...
- **受控九宫格候选**：默认把用户描述包装为服务端 prompt，一次生成 3×3 纯绿幕候选图，自动切图、抠绿幕，并由 VL 给 9 个候选评分排序；9 个候选都会生成最终像素产物，最高分候选作为兼容主输出
...
├── candidate_outputs/      # 9 个候选各自的最终像素图和预览图
