...
主页「范例图鉴」包含物品图标、真实上游实测样例、平铺纹理和序列帧；登录后还会出现「用户分享」tab，仅展示管理员审核通过的用户作品，并可按实际输出尺寸、生图模型和直出类型快速筛选。序列帧分享会在卡片进入可视区域后按帧播放，并兼容历史分享只有横向 sprite sheet、没有逐帧 URL / 坐标的情况；卡片离屏或页面退到后台时暂停，系统启用「减少动态效果」时固定展示首帧。作品库、任务队列和管理后台「任务与作品」统一复用同一播放器：历史任务可从 `pixelized_url` 与 `frame_count` 恢复播放，宽卡片始终按单帧边界居中裁切，不会露出左右相邻帧。用户分享筛选仍以「序列帧」分类展示；参数按钮使用与作品库一致的分组弹窗展示公开快照。主体类素材的尺寸重试支持前端全部默认像素尺寸档位（16/24/32/48/64/96/128/256），按透明成品尺寸匹配 `pixelize.output_size`。用户在作品库点击「提交审核」后，作品先进入待审核队列，管理员通过后才会展示在首页、允许其他登录用户点赞/下载，并在通过时发放分享奖励。实测样例会展示本地真实流程生成的 Logo / 技能书结果，并在卡片和筛选器中标注使用的生成模型（如 `image2`、`gemini-3.1-flash-image-preview`），静态图片位于 `apps/web/public/homepage-examples/showcase/`。
...
SMTP 投递失败时，注册与找回密码页面会按“邮箱不存在或不可用、收件箱已满、收件方拒收、邮件服务繁忙、连接失败、服务配置异常”等情况显示可操作的友好提示；后台测试邮件会提供对应的配置检查建议。SMTP 状态码、服务器原始响应和异常结构不会返回给浏览器，仅在服务端日志中保留脱敏后的诊断信息。
...
登录后，桌面端右上角会并排显示当前可用点数与「充值」按钮；点击后直接进入 `#/billing` 点数中心选择固定套餐、自定义点数或月卡。窄屏会收起这组快捷信息，但账号菜单中的「点数中心」仍保留完整充值入口。
...
「概览」升级为可筛选、可对比的专业运营总览：支持 `24h / 7d / 14d / 30d / 90d / 自定义`（最长 365 天），自动使用小时、日或 7 天粒度，也可在合法范围内手动切换；默认展示最近 14 天并与上一等长周期比较。范围、粒度、对比开关、自定义日期和趋势主题都会写入 `#/admin?...`，刷新、前进后退或复制链接后可恢复。顶部六项主指标聚焦任务量、成功率、消费点数、充值点数、付费订单和活跃用户；主趋势可在任务质量、点数流量、订单转化、用户活跃间切换，当前期使用实线、上一期使用虚线，并通过精确 Tooltip、可开关图例和逐时 / 逐日明细联动继续下钻。实时运行、异常诊断和全量累计账本仍保留，但降为辅助层，避免与周期趋势争夺视觉优先级。

所有周期和「今日」指标都按**站点时区**（系统设置 `site.timezone`，默认 `Asia/Shanghai` = UTC+8）切分。充值仅汇总已支付订单的点数，不把注册赠送或管理员补点计入；活跃用户按注册、创建任务、上传、创建订单或支付订单的用户并集去重；新订单按 `created_at`，付费订单按 `paid_at`，支付率则使用周期内创建且在周期结束前完成支付的订单，避免跨期支付流量扭曲转化口径。无记录桶会补零；上一期没有真实数据时，前端只展示本期，不生成虚假的变化百分比。

`GET /admin/dashboard` 接受以下管理员查询参数，并继续保留原有全量累计、今日字段及 `history/history_days` 兼容字段：

| 参数 | 默认值 | 说明 |
|---|---|---|
| `range` | `14d` | `24h`、`7d`、`14d`、`30d`、`90d` 或 `custom`。 |
| `granularity` | `auto` | `auto`、`hour`、`day`、`week`；小时粒度最多 7 天。 |
| `compare` | `true` | 是否返回紧邻本期之前的上一等长周期。 |
| `from` / `to` | — | `range=custom` 时必填的站点本地日期（`YYYY-MM-DD`，含首尾）。 |

响应新增 `window`、`current_period`、`previous_period`、`series` 与 `previous_series`；后端直接读取受时间边界约束的用户、任务、订单和上传业务记录，不创建易失真的持久化统计表。
...
OpenAI Images 兼容模型走 `/v1/images/generations` / `/v1/images/edits` 或 `image_input` payload；Midjourney、Kling 这类异步协议会提交任务后轮询查询端点；胜算云（`shengsuanyun` 协议）用 OpenAI gpt-image 兼容请求体走异步任务流程（`POST /api/v1/tasks/generations` 提交、`GET /api/v1/tasks/generations/{id}` 轮询，图生图复用同端点仅多传 `image` 字段，结果只返回图片 URL）；Ideogram/FAL 使用各自专用路径。默认优先要求返回 `b64_json` 并直接落盘；如果上游只返回临时 `url`，才作为兼容兜底下载。模型配置可用 `requires_public_image_url=true` 声明参考图字段只接受 HTTP(S) URL（Crazyrouter 的 Gemini 原生兼容端点即属于此类）；Web worker 会基于 `PIX_WEB_PUBLIC_BASE_URL` 生成短时、绑定当前用户且仅允许访问该单一文件的 `/files` URL，未声明时继续发送 base64 data URL。生产环境启用该能力时，`PIX_WEB_PUBLIC_BASE_URL` 必须是上游可回源访问的 HTTPS API 地址。为控制上游成本，素材任务默认关闭多候选图；仅在后台或配置显式开启 `image_gen.contact_sheet_enabled` 时，才会按 `n_sample_count` 发起多候选请求。
