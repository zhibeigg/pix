...
2. 后端先审核原始描述，再把它包装为受控 prompt：默认生成 **3×3 动态纯色抠色背景九宫格候选图**
3. Python 自动切出 9 个候选，按 prompt 动态选择 key color，抠除纯色背景并裁剪主体
...
├── 01_contact_sheet.png    # 默认受控生图的 3×3 纯色抠色候选总图
├── 01_candidate_scores.json # VL 对 9 个候选的分数、排序和选择原因
├── candidates/             # 抠色背景移除后的候选 candidate_01.png ... candidate_09.png
