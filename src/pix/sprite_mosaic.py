...
def _fallback_mosaic_prompt(**values: Any) -> str:
    return (
        ...
        f"First decide the subject's full palette, THEN keep {values['green']} as the background only if it MAXIMIZES the "
        "MINIMUM RGB Euclidean distance to EVERY visible subject color; otherwise shift the whole design so no subject color "
        f"comes near {values['green']}, strongly preferring saturated subject hues opposite/complementary to {values['green']}. "
        f"This minimum distance must be far greater than the key-color tolerance ({values['key_tolerance']} RGB Euclidean distance), "
        "targeting at least 150 RGB Euclidean distance, so the background never blends with any subject color. "
