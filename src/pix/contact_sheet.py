...
def build_sample_prompt(
    ...
    *,
    ...
    return (
        ...
        f"Use one flat solid key-color {values['green']} for the entire background and fill it edge to edge for chroma-key removal — the background must be perfectly uniform with NO gradient, NO vignette, NO lighting or shading, and NO drawn grid lines or graph paper. First decide the subject's full palette, THEN keep {values['green']} as the background only if it MAXIMIZES the MINIMUM RGB Euclidean distance to EVERY visible subject color; otherwise shift the whole design so no subject color comes near {values['green']}, strongly preferring saturated subject hues opposite/complementary to {values['green']}. This minimum distance must be far greater than the key-color tolerance ({values['key_tolerance']} RGB Euclidean distance), targeting at least 150 RGB Euclidean distance, so the background never blends with any subject color. "
...
def _fallback_prompt(**values: Any) -> str:
    return (
        ...
        "For human characters, make sure the face is flat and no shadow. "
        "The subject must be centered with clear empty pixel rows around all edges for safe sprite padding and clean extraction. "
        f"Use one flat solid key-color {values['green']} for every candidate background and all empty cells for chroma-key removal — each background must be perfectly uniform with NO gradient, NO vignette, NO lighting or shading, and NO drawn grid lines, graph paper, or visible cell borders/separators between candidates. First decide each candidate's full subject palette, THEN keep {values['green']} as the background only if it MAXIMIZES the MINIMUM RGB Euclidean distance to EVERY visible subject color; otherwise shift the whole design so no subject color comes near {values['green']}, strongly preferring saturated subject hues opposite/complementary to {values['green']}. This minimum distance must be far greater than the key-color tolerance ({values['key_tolerance']} RGB Euclidean distance), targeting at least 150 RGB Euclidean distance, so the background never blends with any subject color. "
        "No anti-aliasing or smoothing — every pixel must be a perfect square aligned to the conceptual pixel grid. "
        "The output image should be pixel-perfect, each cell only contains one color. No text, no watermark, no extra frame, no labels."
    )
