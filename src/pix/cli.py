...
def cmd_asset(
    ...
    effective_cleanup = bool(cfg.asset.grid_cleanup if grid_cleanup is None else grid_cleanup)
    effective_outline = bool(cfg.asset.grid_outline if grid_outline is None else grid_outline)
    cfg.asset.grid_cleanup = effective_cleanup
    cfg.asset.grid_outline = effective_outline
    cfg.asset.fit_canvas = effective_fit_canvas
    cfg.asset.fit_mode = effective_fit_mode
    cfg.asset.fit_padding = effective_fit_padding
    cfg.asset.fit_min_axis_coverage = effective_fit_min_axis_coverage
    ...
    if effective_grid_mode:
        ...
        result_grid_path = getattr(result, "grid_path", None)
        if result_grid_path is not None and cfg.asset.grid_json and not no_grid_json:
            shutil.copyfile(result_grid_path, grid_target)
...
def cmd_sprite(
    ...
    key_mode: Optional[str] = typer.Option(None, "--key-mode", help="抠色模式：hard=硬阈值；soft=半透明软抠色并支持去污染"),
    key_tolerance: Optional[int] = typer.Option(None, "--key-tolerance", min=0, max=441, help="抠色硬阈值；默认读 [sprite].green_screen_tolerance"),
    key_softness: Optional[int] = typer.Option(None, "--key-softness", min=1, max=441, help="soft 模式完全不透明距离；默认读 [sprite].key_softness"),
    key_alpha_floor: Optional[int] = typer.Option(None, "--key-alpha-floor", min=0, max=255, help="soft 模式低于该 alpha 的像素直接清除"),
    no_despill: bool = typer.Option(False, "--no-despill", help="soft 模式不做 key color 去污染"),
    no_cache: bool = typer.Option(False, help="禁用缓存"),
    refresh: bool = typer.Option(False, help="忽略缓存命中，强制刷新"),
    ...
    result = run_sprite_pipeline(
        ...
        SpritePipelineInput(
            ...
            key_despill=False if no_despill else None,
        ),
        progress=_progress_printer,
    )
