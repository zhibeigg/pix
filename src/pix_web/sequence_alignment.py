...
def _load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail="序列帧元数据无法读取"
        ) from exc
    if not isinstance(data, dict):
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail="序列帧元数据格式不正确"
        )
    return data
...
def _paste_offset(
    source: Image.Image, size: tuple[int, int], offset: tuple[int, int]
) -> Image.Image:
    ...
    if isinstance(sequence, dict):
        ...
        if isinstance(frame_size, dict):
            try:
                ...
                pass
    raise HTTPException(
        status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail="序列帧缺少有效单帧尺寸"
    )
...
def _sprite_grid(
    sprite_meta: dict[str, Any], raw_frames: list[Any]
...
def _frame_grid_position(
    item: dict[str, Any], frame_index: int, cols: int | None
) -> tuple[int, int]:
    if cols and cols > 0:
...
def _row_entries(
    ...
    frames_by_index = {
        int(item["index"]): item for item in aligned_meta if isinstance(item.get("index"), int)
    }
    ...
    for row_index in range(rows):
        ...
        frame_indices = (
            [int(value) for value in raw_indices if _int_or_none(value) is not None]
        ...
        if not frame_indices:
            frame_indices = [
                ...
                if _int_or_none(item.get("row")) == row_index
            ]
        phase = str(existing_entry.get("action_phase") or "").strip() or _row_phase(
            sprite_meta, row_index, frame_indices, frames_by_index
        )
        result.append(
            {
                "row_index": row_index,
...
def _quantize_aligned_frames(
    images: list[Image.Image], *, colors: int | None, dither: str
...
def _compose_row_outputs(
    ...
    *,
    ...
    for spec in _row_entries(sprite_meta, rows, cols, aligned_meta):
        ...
        row_paths = [
            frame_paths_by_index[index] for index in frame_indices if index in frame_paths_by_index
        ]
        if not row_paths:
            continue
...
def apply_sequence_alignment(
    job: GenerationJob, output: GenerationOutput, payload: SequenceAlignmentRequest
) -> GenerationJob:
    """应用每帧偏移并更新当前输出的活跃序列帧版本。"""

    if job.status != "succeeded":
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail="只有生成成功的序列帧可以调整锚点"
        )
    if job.job_type != "sprite_sheet":
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail="该作品不是序列帧任务"
        )
    ...
    if not isinstance(sprite_meta, dict):
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail="作品缺少序列帧元数据"
        )
    raw_frames = sprite_meta.get("frames")
    if not isinstance(raw_frames, list) or not raw_frames:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail="作品缺少可调整的序列帧"
        )
    ...
    for index, item in enumerate(raw_frames, start=1):
        ...
        if source_path is None or not source_path.is_file():
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=f"第 {frame_index} 帧文件不存在"
            )
    ...
    if not pending_frames:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail="没有可合成的序列帧"
        )

    aligned_images, palette = _quantize_aligned_frames(
        aligned_images, colors=color_count, dither=dither
    )
    aligned_frame_paths: list[Path] = []
    aligned_frame_paths_by_index: dict[int, Path] = {}
    aligned_meta: list[dict[str, Any]] = []
    for sheet_position, (item, aligned) in enumerate(
        zip(pending_frames, aligned_images, strict=False)
    ):
    ...
    if rows and cols and len(aligned_frame_paths) == rows * cols:
        ...
        compose_grid_sprite_sheet(
            aligned_frame_paths, grid_path, rows=rows, cols=cols, frame_size=frame_size
        )
        grid_rel = _rel(grid_path, run_dir)

    original_outputs = meta.get("outputs") if isinstance(meta.get("outputs"), dict) else {}
    original_alignment = (
        sprite_meta.get("alignment") if isinstance(sprite_meta.get("alignment"), dict) else None
    )
    alignment = {
        "version": version,
    ...
    sprite_meta["row_sheets_dir"] = (
        _rel(align_root / "row_sheets", run_dir)
        if any(entry.get("sheet") for entry in rows_outputs)
        else None
    )
    sprite_meta["row_previews_dir"] = (
        _rel(align_root / "previews", run_dir)
        if any(entry.get("gif") for entry in rows_outputs)
        else None
    )
    ...
    versions.append(
        {
            "version": version,
            "path": _rel(alignment_path, run_dir),
    ...
    if color_count is not None:
        ...
        effective_params = (
            pixelize_meta.get("effective_params")
            if isinstance(pixelize_meta.get("effective_params"), dict)
            else {}
        )
