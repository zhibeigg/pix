...
- 管理后台「用户与点数」新增创建账户表单，管理员可填写邮箱、可选昵称和临时密码，创建后立即刷新用户列表并按当前注册赠送规则初始化点数账户。
- 新增管理员资源接口 `POST /admin/users`，固定创建普通激活用户，复用注册密码策略、Argon2 哈希和邮箱唯一性校验，并对重复邮箱返回 `409`。
...
- SMTP 投递异常不再把 `550`、收件邮箱、服务器原始响应或 Python 异常结构直接显示给用户；注册验证码、密码重置和后台测试邮件现在会区分邮箱不存在、收件箱已满、收件方拒收、服务繁忙、连接失败、认证失败及 TLS 配置错误，并给出对应的友好处理建议。
- 注册与密码重置邮件发送失败时继续回滚验证码事务；完整 SMTP 诊断仅以收件地址脱敏后的结构化日志保留，兼顾用户体验与运维排障。
- 管理后台「邮件验证码」测试按钮补齐发送中状态与失败捕获，SMTP 配置建议现在会在表单内明确展示，不再形成未处理的 Promise 异常。
...
- 管理后台概览升级为「运营总览 2.0」：支持 24 小时、7/14/30/90 天和最长 365 天自定义范围，自动或手动切换小时/日/7 天粒度，并可与上一等长周期比较。
- 新增任务质量、点数流量、订单转化、用户活跃四类可切换趋势，以及可开关图例、十字线 Tooltip、图表与逐桶明细联动、浅深主题和移动端日期列表。
- `GET /admin/dashboard` 新增周期窗口、当前/上一周期摘要和零填充序列；活跃/付费用户按周期去重，订单支付率使用周期内创建且及时完成支付的订单，并为 `payment_orders.paid_at` 增加查询索引。

### Changed

- 概览主层收敛为任务量、成功率、消费点数、充值点数、付费订单、活跃用户六项运营 KPI；实时故障诊断和全量累计数据压缩为辅助状态带与账本。
- 仪表盘范围、粒度、对比开关、自定义日期和趋势主题写入管理后台 hash，刷新、前进后退和复制链接均可恢复；查询刷新失败时保留最后一次成功数据并支持重试。

## [1.132.9] - 2026-07-14

### Fixed

- 修复管理后台概览只能查看当日指标、历史运营数据虽仍保存在数据库却无法展示的问题；新增任务、成功/失败、充值/消费点数、订单、付费订单和上传的累计统计，并按站点时区返回最近 14 个自然日的逐日历史，自动为无记录日期补零。
- 概览前端按「今日运行 / 历史累计 / 最近 14 天」分层展示，保留原有 30 秒轮询、手动刷新与页面恢复可见时刷新逻辑，并同步补齐中英文文案和窄屏布局。
...
- 生产工作台「游戏素材直出」的参考图上传支持多选：普通素材 / UI / Logo / 角色可一次上传最多 8 张参考图，提交时按参考图数量创建同参数图生图任务，每张任务使用对应参考图并独立排队、计费、进入作品库；无参考图时仍可用「生成数量」手动抽多张。

## [1.124.0] - 2026-07-05

### Added

- 作品库非序列帧成功作品卡片新增「微调」按钮：一键回到生产工作台单图模式，把该作品的成品像素图自动载入为参考图（素材 + 图生图 image_to_image，按 `image_to_image` 计费），沿用原作品素材类型（平铺纹理 / 双瓦片等不支持参考图的类型回落为物品图标）、像素尺寸、颜色数与模型，提示词默认填「保留主体，优化材质和颜色」，改写后即可二次编辑出图。

### Removed

- 移除作品库右侧「微调工位」（TuningPanel）及其承载的免费「本地像素化」侧栏与序列帧「精灵表信息」侧栏；微调能力改由作品卡片「微调」按钮承接，作品库改为单列全宽网格。序列帧的对齐 / 下载 / 预览等能力仍在卡片内保留。

## [1.123.0] - 2026-07-05

### Added

- 作品库多选模式新增「下载所选」：勾选多个已完成作品后可一次性打包成 ZIP 下载。新增后端 `POST /jobs/bulk-download`，按作品类型收录交付物（普通素材的源图 / 像素成品、序列帧的 sheet/GIF/mosaic/sequence.json、双瓦片图集与预览），每个作品放入独立子目录并附带 meta.json；仅本人、已成功且有产物的作品会被计入，全部无产物时返回 409。前端多选工具栏在「删除所选」旁新增「下载所选」按钮与打包中状态。

## [1.122.1] - 2026-07-05

### Fixed

- 作品库失败任务「重试」改为原地复用同一作品卡片重跑，不再新建作品：后端 `POST /jobs/{id}/retry` 直接把失败任务重置为 `pending`、清理旧产物、按当前价格重新冻结点数并重新入队，作品库不再多出一张卡片；重试后仍保持原任务 ID，也不再占用作品库容量额度。

## [1.122.0] - 2026-07-05

### Added

- 作品库为每个已完成作品新增「再生成一张」按钮：用原作品的提示词与参数直接快速重新生成一份新任务，无需回到工作台手动再点生成。新增后端 `POST /jobs/{id}/regenerate` 接口，面向任意已完成作品（成功 / 失败 / 取消），沿用当时完整配方（含界面未暴露的高级像素参数与风格档案）并按当前价格冻结点数。

### Fixed

- 修复复用作品时像素尺寸错位：三视图拼合、透明补边、尺寸重试会让成品实际尺寸大于用户当时选择的单视图 / 单帧尺寸，此前复用会回填这个被放大的产物尺寸。现在尺寸只还原用户当时在表单里选择的请求配方，缺失时回落到模式默认值而不再采用产物尺寸。

## [1.121.0] - 2026-07-05

### Changed

- 点数中心月卡购买卡片升级为铜 / 银 / 金真实金属质感通行证视觉，加入拉丝金属纹理、高光扫光、档位权益徽章和更强的购买 CTA。
- 月卡卡片新增价值转化文案：按永久点数 10 点/元展示「最快回本天数」「30 天相当于多少钱」和回本后剩余天数提示，强化购买动机且保持真实可计算。

## [1.120.0] - 2026-07-05

### Added

- 新增月卡会员功能：默认铜卡 100 点/天 ¥99、银卡 200 点/天 ¥199、金卡 300 点/天 ¥299，时长 30 天；再次购买会顺延有效期并可切换档位。每日临时额度按站点业务时区自然日刷新，当天未用完会清零。
- 生成任务扣点新增「临时额度优先」钱包：任务创建时优先冻结当日临时额度，不足部分再冻结永久点数；任务成功确认消费，失败同日退回临时额度，跨日临时额度退款作废并记录流水备注。作品库/素材包扩容等一次性消费仍只使用永久点数。
- 新增 `/membership/plans`、`/membership/me`、`/membership/checkout` 会员接口，支付订单支持 `order_kind=membership`，支付成功后自动激活/续期会员；管理后台新增月卡档位 CRUD。
- 前端点数中心新增月卡购买区块、今日临时额度/总可用展示、月卡订单展示和临时额度流水筛选；顶部与工作台可用余额使用「临时额度 + 永久点数」合计。

### Documentation

- README 和 API 说明补充月卡会员、每日临时额度、余额字段与消费优先级。
...
- video_bridge 价格与时长档位补齐为飞书价格计算器 4–15 秒完整表：Standard 47/57/66/75/84/94/103/112/121/131/140/149，Fast 40/48/55/62/70/77/85/92/100/107/114/122，Mini 29/34/38/43/47/52/57/61/66/70/75/80；6/7/8/9/11/12/13/14 秒不再向上吸附到 10/15 秒。

## [1.109.2] - 2026-07-02

### Fixed

- 修正 Seedance 2.0 视频模型枚举为 Standard / Fast / Mini（`doubao-seedance-2-0-260128`、`doubao-seedance-2-0-fast-260128`、`doubao-seedance-2-0-mini-260615`），移除错误的 Lite / Pro ID。
- video_bridge 点数改为按官方 4/5/10/15 秒价格表精确计算：Standard 47/57/103/149，Fast 40/48/85/122，Mini 29/34/57/80；默认时长吸附档位同步收敛为 4/5/10/15 秒，避免 6/8/12 秒线性估算。

## [1.109.1] - 2026-07-02

### Added

- 连贯动作序列帧支持在工作台与外部 API 选择 Seedance 2.0 视频模型，并通过 `sprite.video_model` 透传给 Ark 视频任务；任务等待状态会记录实际使用的 `model` / `video_model`。

### Changed

- 生产工作台默认进入 `sprite.mode="video_bridge"` 的连贯动作序列帧模式（后端 schema 默认仍兼容 `mosaic`），并默认使用丝滑动作 16 帧预设。
- 视频补间计费改为所选 Seedance 模型单任务价：按火山/飞书价格表 480p、输入不含视频价格执行 `ceil(视频价格 × 20 + 10)`；不再按帧组数乘序列帧基础价。
...
- 新增公开分享作品机制：用户可将成功作品发布到首页「用户分享」池，作品按点赞数排序展示，支持匿名浏览/下载、登录点赞、查看安全生成参数快照；首次公开按后台「作品分享」设置返还点数，并通过独立下载清单避免任意文件路径暴露。

## [1.96.12] - 2026-06-29

### Fixed

- 修复上传 JPG 参考图后预览破图的问题：用户刚选择文件时不再依赖 `/files`、`blob:` 或 `data:` 图片 URL，而是直接用 `createImageBitmap(file)` 解码本地 File 并绘制到 canvas；上传成功后仅保存后端 `path` 用于任务提交，复用历史任务时仍使用后端文件链接。
- Web 生图任务不再命中 `.pix_cache` 内部生图缓存：原始生图、素材直出、图生图微调和序列帧都会强制调用上游生成；Provider HTTP 请求同步加入 `Cache-Control: no-cache, no-store`，避免中间层复用旧结果。同时隐藏普通用户生图模型下拉中的具体上游供应商信息，并移除参考图成功/模式说明提示。
...
- 修复作品库「复用」回填不完整的问题：现在会从历史任务的主体类型、纹理类型和双瓦片材质字段推断素材类型，避免旧任务或二次微调任务被错误回填为默认物品图标；同时恢复/重置尺寸重试状态，防止复用后带入错误参数。

## [1.95.7] - 2026-06-28

### Fixed

- 修复生成参数弹窗中输入图片只显示“已上传图片”的问题：现在有上传图 URL 时会直接展示缩略图，并可点击打开原图预览。

## [1.95.6] - 2026-06-28

### Fixed

- 修复作品库尺寸重试尝试缩略图的点击行为：现在点击下方任意“尝试”只会在当前作品卡片内切换预览、尺寸标签和下载目标，不再创建新的本地像素化任务或插入额外作品。
...
- 修复生图背景色可能与主体撞色（背景与主体某些颜色过近，既影响观感又导致 chroma-key 抠图误伤主体）的问题：把所有生图模板的背景色约束从「与主体色距离 > 抠色容差（约 48）」这一弱约束，升级为 **maximin** 选色——要求模型先确定主体完整调色板，再选一个使「与**每一个**主体可见颜色的 RGB 欧氏距离中的**最小值最大化**」的背景色，并优先主体完全没用到的高饱和互补 / 对立色相；最小距离目标提高到 **≥150 RGB 欧氏距离**（远大于抠图 `pixel_bg` 的生长阈值，留足余量）。覆盖素材直出默认模板与 `asset.py` canonical 兜底、候选生成（`contact_sheet` / `n_sample`）模板与 `contact_sheet.py` 两处兜底、序列帧 `mosaic` 模板与 `sprite_mosaic.py` 兜底，并同步 `config.example.toml`、`README.md`；新增 `tests/test_prompt_background_distance.py` 锁定 maximin 措辞防回退。本次仅强化 prompt 文案，不改抠图算法、`resolve_key_color` 选色与 `green_screen_tolerance` 默认值。
...
- 管理后台「任务与作品」新增作品库视角：可按状态、用户、关键词筛选最新 500 个全站任务，像用户作品库一样预览 / 下载产物，并保留重试、取消退款和失败退款操作列表；`GET /admin/jobs` 的 `JobResponse` 现在包含 `user_id` 便于前端关联任务归属。
...
- 新增胜算云（ShengSuanYun，`shengsuanyun` 协议）生图上游：OpenAI gpt-image 兼容请求体 + 异步任务轮询（`POST /api/v1/tasks/generations` 提交、`GET /api/v1/tasks/generations/{id}` 轮询），承载 `image2`（上游 `openai/gpt-image-2`）文生图与图生图（图生图复用同端点仅多传 `image` 字段）；检测到 `SHENGSUANYUN_API_KEY` 自动注入为生图 Provider 并参与失败切换。
...
- 生图模型选择收敛为 `image2`、`gemini-3.1-flash-image-preview`、`gemini-3-pro-image-preview` 三项；旧 `gpt-image-2` 配置会自动映射到 `image2`，Crazyrouter 模型发现结果也会经过 allowlist 过滤，不再把 Qwen、Doubao、Nano Banana、Midjourney、Ideogram、FAL、Kling 等模型塞进下拉菜单。
...
- 调整生图 Provider 默认 priority 顺序为 Packy=10 → 胜算云=20 → Crazyrouter=30：多 Provider 失败切换的主力由 Crazyrouter 改为 Packy。

### Fixed

- 修复性能监控「Provider 成功率」只按任务最终 provider 字段统计的问题：现在改为「提供商成功率」，以后台已添加的上游供应商为基准展示名称 / 启停状态，并从 provider 尝试历史中统计 fallback 过程中每个供应商的成功 / 失败次数。
- 修复普通素材上传参考图被前端提交成裸 `image_to_image` 的问题：现在仍按 `asset` 走素材直出 prompt 构建，参考图会先作为像素风重绘参考，再套用尺寸、颜色数、纯色背景、禁文字等素材约束，避免只是处理上传图。
- 修复纯色洋红 / 绿幕等 key 背景去除阈值过宽的问题：边框高度纯净时会自适应收紧 `t_grow`，避免棕橙、皮革等主体色与背景相近时被整块误抠除。
- 修复后台「任务与作品」诊断详情展开卡顿：默认只渲染摘要和短预览，完整诊断 JSON / 详细错误改为按需复制，避免超长 traceback 与上游响应一次性塞进 DOM。
- 修复序列帧动作行数被前景投影误判的问题：行数代表用户请求的动作数，不再被自动网格检测从 4 行误降为 3 行；列数检测仍用于纠正模型少画 / 多画列。
...
- 管理后台「系统公告」面板右侧新增双标签预览区，可在「公告卡片」和「验证码卡片」之间切换；下方新增发送测试验证码邮件功能。

### Fixed

- 修复「保存草稿」和「下线公告」按钮行为完全相同导致意外下线公告的问题；保存草稿现在保持当前启用状态不变。
- 公告签名改为包含启用状态：重新启用已下线的公告现在也会触发邮件通知。
- 批量发送公告邮件时 SMTP 断连后自动重连，避免后续收件人全部失败。
- 发布公告时预检 SMTP 配置，未配置时返回明确的 `smtp_not_configured` 原因而不是误报已发送。
- `ensure_default_system_settings` 改用 `flush()` 替代 `commit()`，避免公告发布流程中事务拆分。
- 公告编辑器不再在每次 settings 刷新时覆盖管理员正在编辑的内容。
- 公告编辑器 `save()` 新增错误提示，API 调用失败时显示具体错误信息。
- 公告邮件预览正文正确处理换行符，与实际 HTML 邮件渲染一致。
- 去除发布公告时前后端重复的 Toast 提示。
...
- 序列帧作品的下载弹窗现在只展示序列帧语义明确的文件项，隐藏与「横向精灵表」/「原版网格精灵表」重复的通用「像素尺寸图」/「源图」选项。
- 下载弹窗改为固定视口居中宽度，选项行的勾选框与内容垂直居中，并在文件较多时只滚动列表区域。
