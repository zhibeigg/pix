...
export function AuthPanel({ user, onLogin, onRegister, onRequestRegisterCode, onLocalTestLogin, onLogout, loading, registrationBonusCredits, localTestLoginAvailable, localTestAccountEmail }: AuthPanelProps) {
  ...
  const registrationBonusCopy = registrationBonusCredits > 0 ? `新人注册赠送 ${registrationBonusCredits} 点数` : ''
  ...
  if (user) return <PixPanel eyebrow="账户" title="已进入工位台" description="账户已连接，可以开始创建素材、查看作品和管理点数。" action={<Button variant="outline" onClick={onLogout}>退出</Button>}><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-lg font-semibold">{user.display_name || user.email}</p><p className="text-sm text-muted-foreground">{user.email}</p></div><Badge variant={user.role === 'admin' ? 'default' : 'secondary'}>{user.role === 'admin' ? '管理员' : '创作者'}</Badge></div><Alert className="mt-4" variant="success">身份已验证，任务和点数会在后台持续同步。</Alert></PixPanel>

  return (
    <PixPanel
      eyebrow="账户"
      title={isRegister ? '邮箱验证注册' : '登录工作台'}
      description={isRegister ? registrationBonusCopy || '验证码用于确认创作者邮箱。' : '回到你的像素素材生产线，继续批量生成、筛选和导出。'}
      action={
