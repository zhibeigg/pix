...
export function AuthPanel({ user, onLogin, onRegister, onRequestRegisterCode, onLocalTestLogin, onLogout, loading, registrationBonusCredits, referralCode, localTestLoginAvailable, localTestAccountEmail, turnstileEnabled, turnstileSiteKey }: AuthPanelProps) {
  ...
  const turnstileActive = isRegister && turnstileEnabled && Boolean(turnstileSiteKey)
  ...
  async function requestCode() {
    ...
    try {
      ...
      // token 一次性消费，无论成功失败都重置以便用户重新校验后再次发送
