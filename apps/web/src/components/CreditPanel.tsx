    ...
    <PixPanel eyebrow="点数中心" title="点数账户" description="选择固定套餐，或输入你需要的自定义点数；实际金额以提交时后端计算为准。" action={<Button variant="outline" onClick={onRefresh}>刷新</Button>}>
      <div className="grid gap-6">
        ...
        <section className="grid gap-3">
          <div className="flex flex-wrap items-end justify-between gap-3"><div><h3 className="text-lg font-semibold">固定套餐</h3><p className="text-sm text-muted-foreground">大额套餐可配置优惠，自定义点数按基准单价计算。</p></div><Badge variant="outline">套餐价优先</Badge></div>
          <div className="grid gap-3 md:grid-cols-3">{packages.map((item, index) => <article key={item.key} className={`rounded-lg border p-4 ${index === 0 ? 'border-primary bg-primary/10' : 'border-border bg-card'}`}><div className="grid gap-4"><div><div className="flex items-center gap-2"><h4 className="font-semibold">{item.name}</h4>{index === 0 && <Badge>推荐</Badge>}</div><p className="mt-2 text-3xl font-semibold">{item.credits}<span className="ml-1 text-sm text-muted-foreground">点</span></p><p className="text-sm text-muted-foreground">{money(item.amount_cents, item.currency)}</p></div><div className="flex flex-wrap gap-2"><Button onClick={() => onCheckout(item.key, 'alipay')}>支付宝支付</Button>{isAdmin && <Button variant="ghost" onClick={() => onCreateOrder(item.key)}>创建模拟订单</Button>}</div></div></article>)}</div>
        </section>

        <section className="rounded-lg border border-border bg-card p-5 shadow-sm">
          <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-end">
            <div>
              <div className="flex flex-wrap items-center gap-2"><h3 className="text-lg font-semibold">自定义充值数量</h3><Badge variant="secondary">后端计价</Badge></div>
            ...
            <div className="rounded-lg border border-border bg-muted/35 p-4">
              <p className="text-xs font-semibold uppercase tracking-[.12em] text-muted-foreground">预计支付</p>
        ...
        <section className="grid gap-3"><h3 className="text-lg font-semibold">充值订单</h3>{orders.length === 0 ? <p className="text-sm text-muted-foreground">暂无充值订单。</p> : orders.map((order) => <div key={order.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border bg-card p-4"><div><p className="font-bold">订单 #{order.id}</p><p className="text-sm text-muted-foreground">{order.credits} 点 · {money(order.amount_cents, order.currency)} · {formatDateTime(order.created_at)}</p></div><div className="flex items-center gap-2"><Badge variant={order.status === 'paid' ? 'success' : 'warning'}>{order.status}</Badge>{isAdmin && order.status !== 'paid' && <Button size="sm" onClick={() => onMockPayOrder(order.id)}>模拟支付</Button>}</div></div>)}</section>
        <section className="grid gap-3"><h3 className="text-lg font-semibold">点数流水</h3>{transactions.length === 0 ? <p className="text-sm text-muted-foreground">暂无流水。</p> : transactions.map((tx) => <div key={tx.id} className="flex justify-between gap-3 border-b border-border py-3"><div><p className="font-bold">{tx.type}</p><p className="text-sm text-muted-foreground">{tx.note || '—'} · {formatDateTime(tx.created_at)}</p></div><p className={tx.amount >= 0 ? 'font-semibold text-emerald-600' : 'font-semibold text-destructive'}>{tx.amount > 0 ? `+${tx.amount}` : tx.amount}</p></div>)}</section>
      </div>
    </PixPanel>
  )
}
