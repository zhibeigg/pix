...
import { CopyPlus, Crosshair, Download, FileDown, PackagePlus, RotateCcw, Share2, Trash2, UserPlus, X } from 'lucide-react'
...
export function GalleryGrid({ jobs, subtitle, selectedJobId, retryingJobId = null, galleryQuota = null, showRetentionQuota = true, onExpandGalleryQuota, onSelect, onReuseJob, onCandidatePixelize, onRetryJob, onDeleteJob, onDeleteJobs, onSaveToPack, onSaveAsCharacter, onRemoveFromPack, onSaveSequenceAlignment, onPublishShare, onUnpublishShare, onActiveActionChange, renderJobBadges, draggableSucceeded = false }: GalleryGridProps) {
  const { t } = useI18n()
    ...
    <PixPanel eyebrow={t('gallery.eyebrow')} title={t('gallery.title')} description={subtitle} action={panelAction}>
      {ordered.length === 0 ? <div className="rounded-lg border border-dashed border-border bg-muted/45 p-8 text-center text-muted-foreground">{t('gallery.empty')}</div> : (
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
          {visible.map((job) => <GalleryCard key={job.id} job={job} selected={selectedJobId === job.id} bulkMode={bulkMode} bulkSelected={bulkSelectedIds.includes(job.id)} bulkDisabled={!isBulkDeletableJob(job)} retrying={retryingJobId === job.id} draggable={!bulkMode && draggableSucceeded && job.status === 'succeeded'} onSelect={onSelect} onBulkToggle={toggleBulkJob} onReuseJob={onReuseJob} onCandidatePixelize={onCandidatePixelize} onRetryJob={onRetryJob} onDeleteJob={onDeleteJob} onSaveToPack={onSaveToPack} onSaveAsCharacter={onSaveAsCharacter} onRemoveFromPack={onRemoveFromPack} onSaveSequenceAlignment={onSaveSequenceAlignment} onPublishShare={onPublishShare} onUnpublishShare={onUnpublishShare} onActiveActionChange={onActiveActionChange} renderJobBadges={renderJobBadges} />)}
        </div>
      )}
...
function GalleryCard({ job, selected, bulkMode, bulkSelected, bulkDisabled, retrying, draggable, onSelect, onBulkToggle, onReuseJob, onCandidatePixelize, onRetryJob, onDeleteJob, onSaveToPack, onSaveAsCharacter, onRemoveFromPack, onSaveSequenceAlignment, onPublishShare, onUnpublishShare, onActiveActionChange, renderJobBadges }: { job: GenerationJob; selected: boolean; bulkMode: boolean; bulkSelected: boolean; bulkDisabled: boolean; retrying: boolean; draggable: boolean; onSelect: (job: GenerationJob) => void; onBulkToggle: (job: GenerationJob, checked?: boolean) => void; onReuseJob?: (job: GenerationJob) => void; onCandidatePixelize?: (job: GenerationJob, candidate: ContactSheetCandidate) => Promise<void>; onRetryJob?: (job: GenerationJob) => Promise<void>; onDeleteJob?: (job: GenerationJob) => void | Promise<void>; onSaveToPack?: (job: GenerationJob) => void | Promise<void>; onSaveAsCharacter?: (job: GenerationJob) => void | Promise<void>; onRemoveFromPack?: (job: GenerationJob) => void | Promise<void>; onSaveSequenceAlignment?: (job: GenerationJob, payload: SequenceAlignmentRequest) => Promise<void>; onPublishShare?: (job: GenerationJob) => void | Promise<void>; onUnpublishShare?: (job: GenerationJob) => void | Promise<void>; onActiveActionChange?: (action: SpriteRowAction | null) => void; renderJobBadges?: (job: GenerationJob) => ReactNode }) {
    ...
    >
      ...
      <div className="grid gap-2.5 p-3">
        ...
        <div className="flex flex-wrap gap-1.5 xs:gap-2">
          ...
          {job.status === 'succeeded' && output && onSaveAsCharacter && <Button size="sm" variant="outline" onClick={(event) => { event.stopPropagation(); void onSaveAsCharacter(job) }}><UserPlus />{t('characters.saveFromWork')}</Button>}
