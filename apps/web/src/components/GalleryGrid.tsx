...
export function GalleryGrid({ jobs, subtitle, selectedJobId, retryingJobId = null, onSelect, onCopyPath, onCandidatePixelize, onRetryJob }: GalleryGridProps) {
  ...
  return (
    <PixPanel eyebrow="作品库" title="作品网格" description={subtitle} action={<div className="flex gap-2"><Badge variant="info">{ordered.length} 件</Badge><Badge variant="outline">第 {safePage}/{totalPages} 页</Badge></div>}>
      {ordered.length === 0 ? <div className="rounded-lg border border-dashed border-border bg-muted/45 p-8 text-center text-muted-foreground">暂无作品，先创建一个任务。</div> : (
  ...
  return (
    <article className={`overflow-hidden rounded-lg border bg-card transition-colors hover:shadow-[0_4px_12px_rgba(15,15,15,0.08)] ${selected ? 'border-primary shadow-[0_4px_12px_rgba(15,15,15,0.08)] ring-2 ring-primary/15' : job.status === 'failed' ? 'border-destructive/40' : 'border-border'}`}>
      ...
      <div className="grid gap-3 p-4">
        <div><h3 className="truncate font-semibold">#{job.id} · {jobTypeLabel(job.job_type)}</h3><p className="mt-1 line-clamp-2 text-sm text-muted-foreground">{jobInputSummary(job)}</p></div>
