...
type GalleryGridProps = { jobs: GenerationJob[]; selectedJobId: number | null; subtitle?: string; retryingJobId?: number | null; regeneratingJobId?: number | null; downloadingJobs?: boolean; galleryQuota?: GalleryQuota | null; showRetentionQuota?: boolean; onExpandGalleryQuota?: () => void; onSelect: (job: GenerationJob) => void; onReuseJob?: (job: GenerationJob) => void; onCandidatePixelize?: (job: GenerationJob, candidate: ContactSheetCandidate) => Promise<void>; onRetryJob?: (job: GenerationJob) => Promise<void>; onRegenerateJob?: (job: GenerationJob) => Promise<void>; onDeleteJob?: (job: GenerationJob) => void | Promise<void>; onDeleteJobs?: (jobs: GenerationJob[]) => void | Promise<void>; onDownloadJobs?: (jobs: GenerationJob[]) => void | Promise<void>; onTuneJob?: (job: GenerationJob) => void; onSaveToPack?: (job: GenerationJob) => void | Promise<void>; onRemoveFromPack?: (job: GenerationJob) => void | Promise<void>; onSaveSequenceAlignment?: (job: GenerationJob, payload: SequenceAlignmentRequest) => Promise<void>; onPublishShare?: (job: GenerationJob) => void | Promise<void>; onUnpublishShare?: (job: GenerationJob) => void | Promise<void>; onActiveActionChange?: (action: SpriteRowAction | null) => void; renderJobBadges?: (job: GenerationJob) => ReactNode; draggableSucceeded?: boolean }
...
export function GalleryGrid({ jobs, subtitle, selectedJobId, retryingJobId = null, regeneratingJobId = null, downloadingJobs = false, galleryQuota = null, showRetentionQuota = true, onExpandGalleryQuota, onSelect, onReuseJob, onTuneJob, onCandidatePixelize, onRetryJob, onRegenerateJob, onDeleteJob, onDeleteJobs, onDownloadJobs, onSaveToPack, onRemoveFromPack, onSaveSequenceAlignment, onPublishShare, onUnpublishShare, onActiveActionChange, renderJobBadges, draggableSucceeded = false }: GalleryGridProps) {
  const { t } = useI18n()
  ...
  const selectedDownloadableJobs = useMemo(() => selectedBulkJobs.filter(isBulkDownloadableJob), [selectedBulkJobs])
  ...
  const hasBulkActions = Boolean(onDeleteJobs || onDownloadJobs)
  ...
  function downloadSelectedJobs() {
    if (!onDownloadJobs || selectedDownloadableJobs.length === 0) return
    void onDownloadJobs(selectedDownloadableJobs)
  }

  const panelAction = <div className="flex flex-wrap items-center gap-2"><Badge variant="info">{t('gallery.itemCount', { count: ordered.length })}</Badge>{showRetentionQuota && <Badge variant={galleryUsed >= galleryLimit ? 'warning' : 'outline'}>{t('gallery.retentionQuota', { used: galleryUsed, limit: galleryLimit })}</Badge>}<Badge variant="outline">{t('gallery.page', { page: safePage, total: totalPages })}</Badge>{bulkMode && <Badge variant="secondary">{t('gallery.bulkSelected', { count: selectedBulkJobs.length })}</Badge>}{showRetentionQuota && onExpandGalleryQuota && <Button type="button" size="sm" variant="outline" onClick={onExpandGalleryQuota}>{t('gallery.expandQuotaButton', { price: expandPrice, slots: expandSlots })}</Button>}{hasBulkActions && !bulkMode && <Button type="button" size="sm" variant="outline" disabled={ordered.length === 0} onClick={enterBulkMode}>{t('gallery.bulkSelect')}</Button>}{hasBulkActions && bulkMode && <><Button type="button" size="sm" variant="outline" disabled={visibleDeletableJobs.length === 0 || allVisibleSelected} onClick={selectVisibleJobs}>{t('gallery.bulkSelectPage')}</Button><Button type="button" size="sm" variant="outline" disabled={selectedBulkJobs.length === 0} onClick={clearBulkSelection}>{t('gallery.bulkClear')}</Button>{onDownloadJobs && <Button type="button" size="sm" variant="outline" disabled={selectedDownloadableJobs.length === 0 || downloadingJobs} onClick={downloadSelectedJobs}><FileDown />{downloadingJobs ? t('gallery.bulkDownloading') : t('gallery.bulkDownload', { count: selectedDownloadableJobs.length })}</Button>}{onDeleteJobs && <Button type="button" size="sm" variant="destructive" disabled={selectedBulkJobs.length === 0} onClick={deleteSelectedJobs}><Trash2 />{t('gallery.bulkDelete')}</Button>}<Button type="button" size="sm" variant="ghost" onClick={exitBulkMode}>{t('gallery.bulkExit')}</Button></>}</div>
    ...
    <PixPanel eyebrow={t('gallery.eyebrow')} title={t('gallery.title')} description={subtitle} action={panelAction}>
      {ordered.length === 0 ? <div className="rounded-lg border border-dashed border-border bg-muted/45 p-8 text-center text-muted-foreground">{t('gallery.empty')}</div> : (
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
          {visible.map((job) => <GalleryCard key={job.id} job={job} selected={selectedJobId === job.id} bulkMode={bulkMode} bulkSelected={bulkSelectedIds.includes(job.id)} bulkDisabled={!isBulkDeletableJob(job)} retrying={retryingJobId === job.id} regenerating={regeneratingJobId === job.id} draggable={!bulkMode && draggableSucceeded && job.status === 'succeeded'} onSelect={onSelect} onBulkToggle={toggleBulkJob} onReuseJob={onReuseJob} onTuneJob={onTuneJob} onCandidatePixelize={onCandidatePixelize} onRetryJob={onRetryJob} onRegenerateJob={onRegenerateJob} onDeleteJob={onDeleteJob} onSaveToPack={onSaveToPack} onRemoveFromPack={onRemoveFromPack} onSaveSequenceAlignment={onSaveSequenceAlignment} onPublishShare={onPublishShare} onUnpublishShare={onUnpublishShare} onActiveActionChange={onActiveActionChange} renderJobBadges={renderJobBadges} />)}
        </div>
      )}
...
function isBulkDownloadableJob(job: GenerationJob) {
  return job.status === 'succeeded' && job.outputs.length > 0
}
...
function GalleryCard({ job, selected, bulkMode, bulkSelected, bulkDisabled, retrying, regenerating, draggable, onSelect, onBulkToggle, onReuseJob, onTuneJob, onCandidatePixelize, onRetryJob, onRegenerateJob, onDeleteJob, onSaveToPack, onRemoveFromPack, onSaveSequenceAlignment, onPublishShare, onUnpublishShare, onActiveActionChange, renderJobBadges }: { job: GenerationJob; selected: boolean; bulkMode: boolean; bulkSelected: boolean; bulkDisabled: boolean; retrying: boolean; regenerating: boolean; draggable: boolean; onSelect: (job: GenerationJob) => void; onBulkToggle: (job: GenerationJob, checked?: boolean) => void; onReuseJob?: (job: GenerationJob) => void; onTuneJob?: (job: GenerationJob) => void; onCandidatePixelize?: (job: GenerationJob, candidate: ContactSheetCandidate) => Promise<void>; onRetryJob?: (job: GenerationJob) => Promise<void>; onRegenerateJob?: (job: GenerationJob) => Promise<void>; onDeleteJob?: (job: GenerationJob) => void | Promise<void>; onSaveToPack?: (job: GenerationJob) => void | Promise<void>; onRemoveFromPack?: (job: GenerationJob) => void | Promise<void>; onSaveSequenceAlignment?: (job: GenerationJob, payload: SequenceAlignmentRequest) => Promise<void>; onPublishShare?: (job: GenerationJob) => void | Promise<void>; onUnpublishShare?: (job: GenerationJob) => void | Promise<void>; onActiveActionChange?: (action: SpriteRowAction | null) => void; renderJobBadges?: (job: GenerationJob) => ReactNode }) {
    ...
    >
      ...
      <div className="grid gap-2.5 p-3">
        ...
        <div className="flex flex-wrap gap-1.5 xs:gap-2">
          ...
          {onRegenerateJob && !isActive && <Button size="sm" variant="default" disabled={regenerating} title={t('gallery.quickRegenerateTitle')} onClick={(event) => { event.stopPropagation(); void onRegenerateJob(job) }}><Zap />{regenerating ? t('gallery.regenerating') : t('gallery.quickRegenerate')}</Button>}
          ...
          {onTuneJob && job.status === 'succeeded' && output && job.job_type !== 'sprite_sheet' && <Button size="sm" variant="outline" title={t('gallery.tuneTitle')} onClick={(event) => { event.stopPropagation(); onTuneJob(job) }}><Wand2 />{t('gallery.tune')}</Button>}
