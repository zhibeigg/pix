...
export function SingleGeneratePanel({ pricing, loading, token, onSubmit }: Props) {
  ...
  async function submit(event: FormEvent) {
    ...
    if (isAsset) {
      await onSubmit({ job_type: 'asset', prompt: assetName.trim(), input_image_path: null, client_request_id: crypto.randomUUID(), pixelize: buildAssetPixelize({ output_size: parsedPixelSize, colors, remove_bg: removeBg, ...edge }), grid: buildGridDesign(), asset: { name: assetName.trim(), extra_prompt: assetExtraPrompt.trim(), no_preview: false } })
      return
    }
