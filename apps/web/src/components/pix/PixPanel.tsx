...
export function PixPanel({ title, eyebrow, description, action, children, className }: { title?: ReactNode; eyebrow?: ReactNode; description?: ReactNode; action?: ReactNode; children?: ReactNode; className?: string }) {
  return (
    <section className={cn('overflow-hidden rounded-lg border border-[hsl(var(--pix-paper-border))] bg-card shadow-[0_10px_30px_-24px_rgba(15,15,15,0.28)] dark:border-border dark:shadow-[0_1px_2px_rgba(15,15,15,0.04)]', className)}>
      {(title || eyebrow || description || action) && (
        <header className="flex flex-col gap-3 border-b border-[hsl(var(--pix-paper-border))] bg-[hsl(var(--pix-paper)/.45)] p-6 md:flex-row md:items-start md:justify-between dark:border-border dark:bg-transparent">
