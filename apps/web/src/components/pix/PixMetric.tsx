...
export function PixMetric({ label, value, tone = 'default', className }: { label: ReactNode; value: ReactNode; tone?: 'default' | 'success' | 'warning' | 'danger' | 'info'; className?: string }) {
  return (
    <div className={cn('rounded-lg border border-border bg-card p-4', tone === 'success' && 'bg-[hsl(var(--pix-mint))] text-[hsl(var(--pix-charcoal))]', tone === 'warning' && 'bg-[hsl(var(--pix-peach))] text-[hsl(var(--pix-charcoal))]', tone === 'danger' && 'bg-red-500/10', tone === 'info' && 'bg-[hsl(var(--pix-sky))] text-[hsl(var(--pix-charcoal))]', className)}>
