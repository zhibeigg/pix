  ...
  return (
    <PixPanel eyebrow="素材包生产" title="批量生产" action={<Badge variant={insufficientCredits ? 'danger' : 'info'}>{taskCount} 个 · 预计 {totalPrice} 点</Badge>}>
      <form className="grid gap-5" onSubmit={submit}>
        ...
        {batchMode === 'asset' || batchMode === 'text_to_image' ? <div className="grid gap-4">{isAsset && <PixField label="统一额外风格描述"><Textarea value={assetExtraPrompt} rows={3} onChange={(e) => setAssetExtraPrompt(e.target.value)} /></PixField>}<PixField label={isAsset ? '素材名称（每行一个）' : '素材描述（每行一个）'}><Textarea value={prompts} rows={8} onChange={(e) => setPrompts(e.target.value)} /></PixField></div> : <div className="grid gap-4 rounded-lg border border-border bg-muted/45 p-4">{batchMode === 'image_to_image' && <PixField label="共用微调描述"><Textarea value={sharedPrompt} rows={4} onChange={(e) => setSharedPrompt(e.target.value)} /></PixField>}<Button type="button" variant="outline" asChild><label className="cursor-pointer"><Upload />{uploading ? '上传中…' : '批量上传图片'}<input type="file" multiple accept="image/png,image/jpeg,image/webp" className="hidden" onChange={(e) => void uploadFiles(e.currentTarget.files)} /></label></Button><UploadList uploads={uploads} /></div>}
...
function UploadList({ uploads }: { uploads: BatchUpload[] }) {
  ...
  return <div className="grid gap-2">{uploads.map((item) => <div key={item.id} className="grid grid-cols-[64px_minmax(0,1fr)] gap-3 rounded-lg border border-border bg-card p-2">{item.upload?.url ? <img src={item.upload.url} alt={item.name} className="h-16 w-16 rounded-md object-contain [image-rendering:pixelated]" /> : <PixPreviewFrame className="min-h-16" label={item.status} />}<div className="min-w-0 self-center"><p className="truncate text-sm font-bold">{item.name}</p><p className="truncate text-xs text-muted-foreground">{item.error || item.upload?.path || item.status}</p></div></div>)}</div>
}
