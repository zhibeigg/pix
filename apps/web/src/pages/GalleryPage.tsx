...
export const GalleryPage = React.memo(function GalleryPage({ jobs, selectedJob, selectedJobId, pricing, loading, retryingJobId, galleryQuota, onExpandGalleryQuota, onSelectJob, onReuseJob, onCandidatePixelize, onCreateJob, onRetryJob, onDeleteJob, onSaveSequenceAlignment }: GalleryPageProps) {
  const { t } = useI18n()
  ...
  return (
    <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(320px,420px)]">
      <div className="grid min-w-0 gap-6">
        ...
        <GalleryGrid jobs={jobs} subtitle={t('pages.gallery.allWorks')} selectedJobId={selectedJobId} retryingJobId={retryingJobId} galleryQuota={galleryQuota} onExpandGalleryQuota={onExpandGalleryQuota} onSelect={onSelectJob} onReuseJob={onReuseJob} onCandidatePixelize={onCandidatePixelize} onRetryJob={onRetryJob} onDeleteJob={onDeleteJob} onSaveSequenceAlignment={onSaveSequenceAlignment} onActiveActionChange={setActiveAction} />
      </div>
