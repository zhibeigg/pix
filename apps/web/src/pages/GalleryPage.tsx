...
export function GalleryPage({ jobs, selectedJob, selectedJobId, pricing, loading, onSelectJob, onCopyPath, onCandidatePixelize, onCreateJob, onRefresh }: GalleryPageProps) {
  return (
    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: 'minmax(0, 1fr) minmax(320px, 420px)' }, gap: 3, alignItems: 'start' }}>
      <Stack spacing={3} sx={{ minWidth: 0 }}>
        ...
        <JobList jobs={jobs.filter((job) => job.status !== 'succeeded')} onRefresh={onRefresh} onCandidatePixelize={onCandidatePixelize} />
      </Stack>
      <Box sx={{ minWidth: 0 }}>
