    ...
    <form className="grid gap-5" onSubmit={submit}>
      <PixPanel eyebrow="Raw Forge" title="原始生图" description="只做文生原图：调基础参数、提交 prompt、在画布里看源图。" action={<div className="flex flex-wrap gap-2"><Badge variant="outline">余额 {balance?.available_credits ?? '—'} 点</Badge><Badge variant={insufficientCredits ? 'danger' : 'info'}>预计 {estimatedCredits} 点</Badge><Button type="button" variant="outline" onClick={() => void onRefresh()}><RefreshCw />刷新</Button></div>}>
        <div className="grid gap-5 lg:grid-cols-[260px_minmax(0,1fr)_148px]">
          ...
          <div className="grid gap-4 rounded-3xl bg-[hsl(var(--pix-navy))] p-4 text-white">
            <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[.14em] text-white/55">Preview Deck</p><h3 className="text-xl font-black">{selectedJob ? `#${selectedJob.id} · ${mainImageLabel}` : '等待第一张原图'}</h3></div>{selectedJob && <PixStatusBadge status={selectedJob.status} />}</div>
          ...
          <div className="grid content-start gap-2 rounded-2xl border border-border bg-card p-3"><p className="text-xs font-black uppercase tracking-[.12em] text-muted-foreground">最近</p>{thumbs.length ? thumbs.slice(0, 8).map((thumb) => <button key={thumb.key} type="button" title={thumb.label} onClick={() => { onSelectJob(thumb.job.id); setPreviewOverride(thumb.candidate ? { jobId: thumb.job.id, url: thumb.url, label: thumb.label } : null) }} className={`overflow-hidden rounded-xl border p-1 ${thumb.selected ? 'border-primary bg-primary/10' : 'border-border bg-muted/30'}`}><img src={thumb.url} alt={thumb.label} className="aspect-square w-full object-cover [image-rendering:pixelated]" /></button>) : <div className="rounded-xl border border-dashed border-border p-4 text-center text-xs text-muted-foreground">暂无缩略图</div>}</div>
        </div>
      </PixPanel>
