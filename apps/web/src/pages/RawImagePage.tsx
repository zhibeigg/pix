...
export function RawImagePage({ pricing, discount, balance, jobs, loading, token, imageModels, selectedJobId, reuseSeed, onSelectJob, onCreateJob, onRefresh }: Props) {
  ...
  useEffect(() => {
    ...
    setReferenceRemoteUrl(signedFileUrl(job.input_image_url ?? undefined, token, true))
    setRefMessage('')
  }, [availableImageModels, imageModels.default, reuseSeed, text, token])

  function setReferenceRemoteUrl(url: string) {
  ...
  async function uploadReferenceFile(file: File | undefined) {
    ...
    setRefUploading(true); setRefMessage('')
    try {
      const uploaded = await api.uploadImage(token, file)
      setRefImagePath(uploaded.path); setRefMessage('')
    } catch (error) {
      setRefImageFile(null)
  ...
  function clearReference() {
    setRefImagePath(''); setRefImageUrl(''); setRefImageFile(null); setRefMessage('')
  }
      ...
      >
        <div className="grid gap-5 lg:grid-cols-[260px_minmax(0,1fr)_148px]">
              ...
              <div className="grid gap-2">
                ...
                {(refImageUrl || refImageFile) && (
                  <div className="grid gap-1">
                    <PixPreviewFrame url={refImageUrl} file={refImageFile} label={text('参考图预览', 'Reference preview')} className="min-h-24" />
                    <Button type="button" variant="ghost" size="sm" onClick={clearReference}>{text('移除参考图', 'Remove reference')}</Button>
                  </div>
                )}
              </div>
            </PixField>
            {hasReference && !modelSupportsI2I && <Alert variant="warning">{text('当前模型不支持参考图 / 图生图，请切换支持 image-to-image 的模型或移除参考图。', 'The selected model does not support reference images / image-to-image. Switch to a model with image-to-image support or remove the reference.')}</Alert>}
          </div>
          ...
          <div className="grid content-start gap-2 rounded-lg border border-border bg-card p-3 dark:border-[hsl(var(--pix-dark-hairline))] dark:bg-[hsl(var(--pix-dark-card))]">
            ...
            {thumbs.length ? <div className="flex gap-2 overflow-x-auto pb-1 lg:grid lg:gap-2 lg:overflow-visible lg:pb-0">{thumbs.slice(0, 8).map((thumb) => <button key={thumb.key} type="button" title={thumb.label} onClick={() => onSelectJob(thumb.job.id)} className={`w-20 shrink-0 overflow-hidden rounded-lg border p-1 lg:w-auto ${thumb.selected ? 'border-primary bg-primary/10 dark:bg-primary/18' : 'border-border bg-muted/30 dark:border-[hsl(var(--pix-dark-hairline))] dark:bg-[hsl(var(--pix-dark-band-soft))]'}`}><img src={thumb.url} alt={thumb.label} className="aspect-square w-full object-cover" /></button>)}</div> : <div className="rounded-lg border border-dashed border-border p-4 text-center text-xs text-muted-foreground">{text('暂无缩略图', 'No thumbnails yet')}</div>}
          </div>
        </div>
      </PixPanel>
