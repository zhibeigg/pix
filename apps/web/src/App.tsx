...
import { isRawImageJob } from './lib/jobReuse'
...
export function App({ themeMode, themePreference, systemThemeMode, language, onThemePreferenceChange, onLanguageChange }: AppProps) {
  ...
  const reuseJobInWorkbench = useCallback((job: GenerationJob) => {
    ...
    // 原始生图 / 图生图原图：回填到原始生图面板，而不是塞进素材直出工作台。
    if (isRawImageJob(job)) {
      setRawReuseSeed({ revision, job })
  ...
  const { createPaymentOrder, startCheckout, createCustomPaymentOrder, startCustomCheckout, createMembershipOrder, startMembershipCheckout, mockPayPaymentOrder } = useBillingActions({ token, refreshCore, setMessage, showError, text, setCheckout })
      ...
      {setupLoading ? (
        ...
        >
          <Suspense fallback={<div className="grid min-h-[calc(100vh-160px)] place-items-center px-4 text-sm text-muted-foreground">{t('app.checkingSetup')}</div>}>
            ...
            {page === 'admin' && isAdmin && <AdminPage token={token} onNotify={setMessage} />}
          </Suspense>
        </WorkspaceShell>
      ) : (
        <div>
