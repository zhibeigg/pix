...
- Asset-output forms should not ask users to write full prompts. They expose structured choices for asset type (item icon / UI component), subject type (single prop / single UI), and one subject input that maps to the backend template’s `主体：{name}` slot; optional style notes stay secondary.
