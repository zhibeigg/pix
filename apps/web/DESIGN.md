...
- User asset packs are manual, persistent containers: batch generation lands in the gallery, then successful works can be saved into packs by standard browser drag-and-drop or the button fallback. Each pack defaults to 100 works; accounts start with 1 creatable pack and each +1 pack-count expansion costs 99 credits. The pack panel should feel like a lightweight file explorer: single-click folder rows, a command bar for common actions, and inline renaming instead of prompt dialogs.
- Asset-output forms should not ask users to write full prompts. They expose one asset-type choice (item icon / UI component) and one subject input that maps to the backend template’s `主体：{name}` slot; subject type is derived from the asset type, and optional style notes stay secondary.
- Local background removal is a lightweight utility mode, not a generation mode: keep upload preview first, then expose a two-option algorithm selector (`像素` / `高清`) with short explanatory helper text. Do not show pixel-size or color-count controls in this mode.
...
- Gallery card previews use absolute `object-contain` images constrained to the top preview frame, preserving aspect ratio without spilling into card metadata.
